 ...
 *
 * 아이콘 이름은 DB에 저장된 값(`categories.icon`, 포트폴리오 스킬 아이콘)에서
 * 온다. 지금 UI는 `ICON_LIST`에서 고르게 되어 있지만, 과거 데이터나 API를
 * 직접 호출해 넣은 값은 그 목록 밖일 수 있다.
 *
 * 그래서 네임스페이스 조회 전에 `ICON_LIST`로 거른다. 이 목록이 이 앱이
 * 지원한다고 선언한 아이콘의 단일 출처이므로, 목록 밖 이름은 fallback으로 보낸다.
 */
